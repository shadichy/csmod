...
public class BambooFenceGateBlock extends FenceGateBlock {
    public BambooFenceGateBlock(AbstractBlock.Settings settings) { super(settings);}

    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (world.getDimension().isUltrawarm()) {
            world.setBlockState(pos, ModBlocks.DRIED_BAMBOO_FENCE_GATE.getStateWithProperties(state), 3);
